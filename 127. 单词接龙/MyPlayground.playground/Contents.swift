...
extension String {
    ...
    public func substring(range at: NSRange) -> String {
        if self.count >= at.location + at.length {
            let startIndex = self.index(self.startIndex, offsetBy: at.location)
            let endIndex = self.index(self.startIndex, offsetBy: at.location + at.length)
...
class Solution {
    func ladderLength(_ beginWord: String, _ endWord: String, _ wordList: [String]) -> Int {
        ...
        var searchWords: [(String, Int)] = [(beginWord, 1)]
        var visted: Array<String> = Array()
        visted.append(beginWord)
        while searchWords.count != 0 {
            
            let node = searchWords.first!
            ...
            for i in 0 ..< Len {
                for c in "abcdefghijklmnopqrstuvwxyz" {
                    let newString = word.substring(range: NSRange(location: 0, length: i)) + String(c) + word.substring(range: NSRange(location: i+1, length: Len-i-1))
                    if set.contains(newString) {
                        if newString == endWord {
                            return level+1
                        }
                        set.remove(newString)
                        searchWords.append((newString, level+1))
                    }
                }
            }
        }
...
//    func minus(_ firstWord: String, _ secondWord: String) -> Int {
//        if firstWord.count != secondWord.count {
//            return 0
//        }
//        var count = 0
//        for i in 0 ..< firstWord.count {
//            if firstWord.prefix(i+1).suffix(1) != secondWord.prefix(i+1).suffix(1) {
//                count += 1
//            }
//        }
//        return count
